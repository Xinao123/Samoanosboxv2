...
def format_eta(sent, total, speed):
    if speed <= 0 or sent >= total: return ""
    r = (total - sent) / (speed * 1e6)
    if r < 60: return f"~{int(r)}s"
    if r < 3600: return f"~{int(r/60)}min"
    return f"~{r/3600:.1f}h"


FILE_ICONS = {
    ".zip": ft.Icons.FOLDER_ZIP, ".rar": ft.Icons.FOLDER_ZIP,
    ".7z": ft.Icons.FOLDER_ZIP, ".tar": ft.Icons.FOLDER_ZIP, ".gz": ft.Icons.FOLDER_ZIP,
    ".mp4": ft.Icons.MOVIE, ".mkv": ft.Icons.MOVIE, ".avi": ft.Icons.MOVIE,
    ".mov": ft.Icons.MOVIE, ".webm": ft.Icons.MOVIE,
    ...
    ".png": ft.Icons.IMAGE, ".gif": ft.Icons.IMAGE, ".webp": ft.Icons.IMAGE,
    ".pdf": ft.Icons.PICTURE_AS_PDF,
    ".doc": ft.Icons.DESCRIPTION, ".docx": ft.Icons.DESCRIPTION,
    ".txt": ft.Icons.TEXT_SNIPPET, ".exe": ft.Icons.SETTINGS_APPLICATIONS,
    ".iso": ft.Icons.ALBUM,
}
...
def main(page: ft.Page):
    ...
    def snack(msg, error=False):
        ...
        page.overlay.append(ft.SnackBar(
            ...
            bgcolor=ft.Colors.RED_700 if error else ft.Colors.GREEN_700, open=True))
        page.update()
    ...
    server_field = ft.TextField(label="Servidor", value=cfg["server_url"],
                                prefix_icon=ft.Icons.DNS, border_radius=10)
    name_field = ft.TextField(label="Seu nome", value=cfg.get("username", ""),
    ...
    entry_view = ft.Column([
        ...
        entry_status,
        ft.Row([
            ft.ElevatedButton("Entrar", icon=ft.Icons.ARROW_FORWARD,
    ...
    notification_banner = ft.Container(visible=False, padding=ft.padding.symmetric(horizontal=16, vertical=8))
    ...
    def build_file_tile(f):
        ...
        return ft.Container(
            content=ft.Row([
                ...
                ft.IconButton(ft.Icons.DOWNLOAD_ROUNDED, tooltip="Baixar",
                              icon_color=ft.Colors.GREEN_400 if can_download else ft.Colors.GREY_700,
                              icon_size=20, disabled=not can_download,
                              on_click=lambda e, f=f: do_download(f)),
                ft.IconButton(ft.Icons.DELETE_OUTLINE, tooltip="Remover",
    ...
    def render_files(files):
        ...
        if not files:
            files_column.controls.append(ft.Column([
    ...
    def start_share(file_path, file_name, file_size):
        ...
        # 2. Registra no server (metadados, sem upload)
        try:
        ...
        # 3. Disponibiliza via P2P
        p2p.share_file(file_id, file_path)

        # 4. Salva no config local
        ...
        # 5. Upload pro server em background (fallback)
        def bg_upload():
            try:
        ...
        def run():
            try:
                def on_prog(recv, total, speed):
                    ...
                    download_text.value = f"DL {format_size(recv)}/{format_size(total)} {pct}% | {speed:.1f} MB/s {eta}"
                    page.update()
                ...
                saved = api.download_file(file_info, cfg.get("download_dir", str(Path.home() / "Downloads")),
    ...
    def confirm_delete(file_id, filename):
        ...
        dlg = ft.AlertDialog(modal=True, title=ft.Text("Confirmar"),
                             content=ft.Text(f'Remover "{filename}"?'),
                             actions=[ft.TextButton("Cancelar", on_click=no),
    ...
    def show_settings(e):
        ...
        dlg = ft.AlertDialog(
            ...
            actions=[ft.TextButton("Fechar", on_click=close), ft.ElevatedButton("Salvar", on_click=save)])
        page.overlay.append(dlg)
        dlg.open = True
        page.update()
    ...
    def start_ws():
        ...
        def on_message(ws, message):
            try:
                ...
                if ev == "file_added" and who != api.username:
                    show_notification(f"{who} compartilhou: {data.get('filename','?')} ({format_size(data.get('size',0))})",
        ...
        def run():
            try:
                ws = ws_lib.WebSocketApp(ws_url, on_open=on_open,
    ...
    # ── Restaura arquivos compartilhados de sessões anteriores ──
    ...
    main_view = ft.Column([
        ft.Container(
            content=ft.Row([
                ft.Row([ft.Icon(ft.Icons.CLOUD, size=22, color=ft.Colors.BLUE_400),
                        ft.Text("SamoanosBox", size=17, weight=ft.FontWeight.BOLD),
                        online_chip], spacing=10),
                ft.Row([storage_chip,
                        ...
                        ft.IconButton(ft.Icons.LOGOUT, tooltip="Sair", icon_size=19,
                                      icon_color=ft.Colors.RED_400, on_click=do_logout)], spacing=0),
            ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
            padding=ft.padding.symmetric(horizontal=20, vertical=8),
            border=ft.border.only(bottom=ft.BorderSide(1, "#15ffffff"))),
        notification_banner,
        ft.Container(content=ft.Column([
            ft.Row([ft.Container(content=search_field, expand=True),
                    ft.ElevatedButton("Compartilhar", icon=ft.Icons.SHARE,
                                      style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=10),
        ...
        ft.Container(content=files_column, expand=True, padding=ft.padding.symmetric(horizontal=20)),
